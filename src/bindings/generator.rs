...
impl<'ci> LivekitNodeTsTemplate<'ci> {
    pub fn new(ci: &'ci ComponentInterface, out_disable_auto_loading_lib: bool, sys_ts_main_file_name: &str) -> Self {
        Self {
            ci,
...
pub fn generate_node_bindings(
    ...
    let node_ts_file_contents = LivekitNodeTsTemplate::new(ci, out_disable_auto_loading_lib, sys_ts_main_file_name).render().context("failed to render livekit-node.ts template")?;
